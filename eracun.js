...
// Vrni podrobnosti o stranki z dolocenim id-jem
var strankaZIdjem = function(strankaId, callback) {
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    seja = zahteva.session;
    var strankaId = seja.ime;
    
    //console.log(strankaId);
    
    strankaZIdjem(strankaId, function(stranka) {
      if(stranka) {
        //console.log(stranka[0])
        if (!pesmi) {
          odgovor.sendStatus(500);
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    if(napaka2) potrdiForm("Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.", odgovor);
    else potrdiForm("Stranka je bila uspešno registrirana.", odgovor);
    
    
  });
})

function potrdiForm(sporocilo, odgovor) {
